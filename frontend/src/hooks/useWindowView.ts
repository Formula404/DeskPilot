...
export function useWindowView(): WindowView {
  ...
  useEffect(() => {
    if (!isTauriRuntime()) {
      ...
      if (preview === "floating-ball" || preview === "overlay" || preview === "settings" || preview === "knowledge" || preview === "context-menu") {
        setView(preview);
      }
      return;
    }
    try {
      ...
      if (label === "floating-ball" || label === "overlay" || label === "settings" || label === "knowledge" || label === "context-menu") {
        setView(label);
      }
    } catch {
      setView("overlay");
